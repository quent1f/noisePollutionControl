...
def launch_simulation(N: int, level: int, spacestep: float, wavenumber: float, V_obj: float, mu: float, chi_init=0):        # penser à ajouter un moyen de faire un initial chi différent
    ...
    chi_final, energy, u_final, grad, chi_final_projected, u_final_projected = minimization_algo.optimization_procedure(domain_omega, spacestep, f, f_dir, f_neu, f_rob, beta_pde, alpha_pde, alpha_dir, beta_neu, beta_rob, alpha_rob, Alpha, mu, initial_chi, V_obj, wavenumber, S)
    ...
    preprocessing.plot_chi_domain_with_wall(chi_final, domain_omega, N, is_horizontal=True, title="Optimal Chi Domain with Wall")
    print('End.')
    return energy
