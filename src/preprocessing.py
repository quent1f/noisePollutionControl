...
def plot_chi_domain_with_wall(chi, domain_omega, wall_index, is_horizontal=True, title="Optimal Chi Domain with Wall"):
    """
    Plot the domain with optimal chi values and overlay the chi distribution along a specific wall.
    
    Parameters:
    - chi: Matrix (MxN, dtype=float), optimal chi values.
    - domain_omega: Matrix (MxN), defines the domain and boundary types.
    - wall_index: int, the row or column index for the wall.
    - is_horizontal: bool, True if the wall is horizontal, False if vertical.
    - title: String, title of the plot.
    """
    # Mask out areas of the domain not relevant to chi
    masked_chi = np.ma.masked_where(domain_omega != _env.NODE_INTERIOR, chi)
    
    plt.figure(figsize=(12, 8))
    
    # Plot the 2D chi distribution
    plt.imshow(masked_chi, cmap='viridis', origin='lower')
    plt.colorbar(label='Chi values')
    plt.title(title)
    plt.xlabel('X-axis')
    plt.ylabel('Y-axis')

    # Overlay chi values along the wall
    if is_horizontal:
        plt.plot(range(chi.shape[1]), [wall_index] * chi.shape[1], 'r-', lw=2, label="Wall Chi Distribution")
        plt.scatter(range(chi.shape[1]), [wall_index] * chi.shape[1], c=chi[wall_index, :], cmap='viridis', edgecolor='k', s=100)
    else:
        plt.plot([wall_index] * chi.shape[0], range(chi.shape[0]), 'r-', lw=2, label="Wall Chi Distribution")
        plt.scatter([wall_index] * chi.shape[0], range(chi.shape[0]), c=chi[:, wall_index], cmap='viridis', edgecolor='k', s=100)
